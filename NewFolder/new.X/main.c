...
void I2C_read_multiple(char address, char regadd, unsigned char * data, char length) {
    unsigned char * temp;
    int j;
    temp = data;
    i2c_master_start(); // make the start bit
    ...
    for (j = 0; j < (length - 1); j++) {
        * temp = i2c_master_recv(); // save the value returned
        i2c_master_ack(0);
        temp++;
    }
    LATAbits.LATA4 = 1;
    * temp = i2c_master_recv();
    i2c_master_ack(1); // make the ack so the slave knows we got it
    i2c_master_stop(); // make the stop bit
}
